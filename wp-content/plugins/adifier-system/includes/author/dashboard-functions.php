...
function adifier_top_comment_mark_read(){
	...
	if( adifier_check_comment_post_owner() ){
		delete_comment_meta( $_POST['parent_id'], 'af_unread' );
	}
	else{
	...
	if( empty( $_POST['comment_message'] ) ){
		...
		if( $user_id ){
			...
			$id = wp_insert_comment(array(
				...
				'comment_parent'	=> $_POST['parent_id_reply'],
				'comment_content'	=> $_POST['comment_message'],
			...
			if( $id ){
				...
				$comment_parent = get_comment( $_POST['parent_id_reply'] );
				
				$author_name = adifier_author_name( $user );
				$comment_url = get_the_permalink( $_POST['post_id'] ).'#comment-'.$id;
				$advert_title = get_the_title( $_POST['post_id'] );
				$comment_content = $_POST['comment_message'];
				...
				$response['comment'] = '<div class="ajax-response">
					...
					<p class="ajax-response-content">'.$_POST['comment_message'].'</p>
				</div>';
			}
			else{
