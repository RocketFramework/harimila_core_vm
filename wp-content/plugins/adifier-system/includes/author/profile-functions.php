...
function adifier_change_password(){
	...
	if( empty( $new_password ) || empty( $new_password_repeat ) || empty( $old_password ) ){
		...
		else if( !preg_match("/^(?=.{8,})(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*\\W).*$/", $new_password) && !preg_match("/^(?=.{7,})(((?=.*[A-Z])(?=.*[a-z]))|((?=.*[A-Z])(?=.*[0-9]))|((?=.*[a-z])(?=.*[0-9]))).*$/", $new_password) ){
...
function adifier_login(){
	$username = !empty( $_POST['log_username'] ) ? esc_sql( $_POST['log_username'] ) : '';	
	$password = !empty( $_POST['log_password'] ) ? esc_sql( $_POST['log_password'] ) : '';
...
function adifier_register( $return_response = false ){
	$username 					= !empty( $_POST['reg_username'] ) ? trim( esc_sql( $_POST['reg_username'] ) ) : '';
	$email 						= !empty( $_POST['reg_email'] ) ? trim( esc_sql( $_POST['reg_email'] ) ) : '';
	$password 					= !empty( $_POST['reg_password'] ) ? esc_sql( $_POST['reg_password'] ) : '';
	$password_confirm 			= !empty( $_POST['reg_r_password'] ) ? esc_sql( $_POST['reg_r_password'] ) : '';
	...
	else if( !preg_match("/^(?=.{8,})(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*\\W).*$/", $password) && !preg_match("/^(?=.{7,})(((?=.*[A-Z])(?=.*[a-z]))|((?=.*[A-Z])(?=.*[0-9]))|((?=.*[a-z])(?=.*[0-9]))).*$/", $password) ){
...
function adifier_recover(){
	$password = !empty( $_POST['rec_password'] ) ? esc_sql( $_POST['rec_password'] ) : '';
	$password_confirm = !empty( $_POST['rec_r_password'] ) ? esc_sql( $_POST['rec_r_password'] ) : '';
	$username = !empty( $_POST['rec_username'] ) ? esc_sql( $_POST['rec_username'] ) : '';
	$hash = !empty( $_POST['rec_hash'] ) ? esc_sql( $_POST['rec_hash'] ) : '';
	$email = !empty( $_POST['rec_email'] ) ? esc_sql( $_POST['rec_email'] ) : '';
	$password_length 			= adifier_get_option( 'password_length' );
	$password_length 			= empty( $password_length ) ? 6 : $password_length;
