...
function adifier_packs_message( $account_payment, $pack ){
	if( $account_payment == 'packages' ){
		if( $pack['price'] == '-' ){
			...
			echo sprintf( __( 'Purchase this package to top up your account with <b>%s</b> ads', 'adifier'), $pack['adverts'] );
		}
	}
		...
		if( $pack['price'] == '-' ){
			...
			echo sprintf( __( 'Create your account and get <b>%s</b> %s subscription as a welcome gift', 'adifier'), $time, $period );
		}
		else{
			echo sprintf( __( 'Purchase this package and extend your subscription by <b>%s</b> days', 'adifier'), $pack['days'] );
		}
	}
