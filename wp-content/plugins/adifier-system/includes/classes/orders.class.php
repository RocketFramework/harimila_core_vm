...
class Adifier_Order{
	...
	*/
	...
	static public function get_buyer_information( $order_id, $user_id ){
		$buyer_location = get_post_meta( $order_id, 'buyer_location', true );
		$buyer_name 	= get_post_meta( $order_id, 'buyer_name', true );		
		if( empty( $buyer_location ) ){
			$buyer_location 	= get_user_meta( $user_id, 'location', true );
			$buyer_name			= get_the_author_meta( 'display_name', $user_id );
			update_post_meta( $order_id, 'buyer_location', $buyer_location );
			update_post_meta( $order_id, 'buyer_name', $buyer_name );
		}
		
		$buyer_location['name'] = $buyer_name;
		
		return $buyer_location;
	}
	
	static public function display_buyer_information( $buyer_information ){
		echo !empty( $buyer_information['name'] ) 		? '<div>'.$buyer_information['name'].'</div>' : '';
		echo !empty( $buyer_information['street'] ) 	? '<div>'.$buyer_information['street'].'</div>' : '';
		echo !empty( $buyer_information['city'] ) 		? '<div>'.$buyer_information['city'].'</div>' : '';
		echo !empty( $buyer_information['state'] ) 	  	? '<div>'.$buyer_information['state'].'</div>' : '';
		echo !empty( $buyer_information['country'] ) 	? '<div>'.$buyer_information['country'].'</div>' : '';
	}
	...
	*/
	static public function create_order( $args ){
		...
		if( !empty( $post_id ) ){
			...
			$buyer_name = get_the_author_meta( 'display_name', $order['userId'] );
			update_post_meta( $post_id, 'buyer_name', $buyer_name );
			
			$buyer_location	= get_user_meta( $order['userId'], 'location', true );
