...
function adifier_custom_meta(){
	...
	$reported_by_text = '';
	if( !empty( $_GET['post'] ) ){
		$reported_by = get_post_meta( $_GET['post'], 'advert_report_by', true );
...
function adifier_order_paid_status( $statuses ){
	if( !empty( $_GET['post'] ) && get_post_meta( $_GET['post'], 'order_paid', true ) != 'yes' ){
...
function adifier_validate_price_format( $price, $currency_id = '' ){
	...
	if( !empty( $currency_specs['thousands_separator'] ) && $currency_specs['show_decimals'] == 'yes' && !empty( $currency_specs['decimal_separator'] ) ){
		$regex .= "*";
	}
...
function adifier_edited_term_taxonomy_count( $post_id, $post, $update ){
	if ( $post->post_status === 'trash' || $post->post_type != 'advert' ) {
		return;
	}

	adifier_update_taxonomy_count( $post_id );
}
...
function adifier_update_taxonomy_count( $advert_id ){	
	global $wpdb;

	$taxonomies = get_object_taxonomies( 'advert' );
	$terms = wp_get_object_terms( $advert_id, $taxonomies );
	if( !empty( $terms ) ){
		$term_ids = wp_list_pluck( $terms, 'term_id' );
		foreach( $term_ids as $term_id ){
			$count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(p.ID) FROM {$wpdb->prefix}posts AS p LEFT JOIN {$wpdb->prefix}adifier_advert_data AS aad ON p.ID = aad.post_id LEFT JOIN {$wpdb->prefix}term_relationships AS tr ON aad.post_id = tr.object_id WHERE p.post_status = 'publish' AND aad.expire > %d AND aad.sold = 0 AND tr.term_taxonomy_id = %d", current_time('timestamp'), $term_id) );
			$wpdb->update(
				$wpdb->term_taxonomy,
				array(
					'count' => $count
