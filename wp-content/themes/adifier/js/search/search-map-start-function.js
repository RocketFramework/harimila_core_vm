		...
		$(document).on( 'adifier-search-map-start', function(e, data){
			$.each(data, function(index, item) {
				...
				if( addedIDs.indexOf(item.id) == -1 ){
					...
					var marker = L.marker(latLng, {icon: item.icon }).addTo(map);
			...
			$.each( markers, function(index, data){
				if( data.markers.length > 1 ){
					$.each( data.markers, function(index, marker){
						marker.bindPopup('<div class="infoBox"><ul class="list-unstyled info-box-markers-list"><li>'+data.content.join( '</li><li>' )+'</li></ul></div>');
					...
					data.markers[0].bindPopup( '<div class="infoBox">'+data.content[0]+'</div>' );
				}
			});
